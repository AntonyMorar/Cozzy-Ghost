...
func _throw_message(_spookeado, _amount):
	if _spookeado:
		...
		var scale_amount = clamp(_amount, 0.0,1.0)*2
		var translate_amount = clamp(_amount, 0.0, 1.0)
		...
		tween.tween_property(selected_message, "position", Vector3(0,translate_amount,0), 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
		...
		tween.tween_property(selected_message, "position", Vector3(0,0,0), 0.5).from(Vector3(0,translate_amount,0)).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
	else:
