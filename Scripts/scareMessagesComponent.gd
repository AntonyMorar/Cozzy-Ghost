...
func _throw_message(_spookeado):
	if _spookeado:
		var selected_message = scary_messages.pick_random()
		selected_message.visible = true
		tween = get_tree().create_tween().set_parallel(true)
		tween.tween_property(selected_message, "scale", Vector3(2,2,2), 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
		tween.tween_property(selected_message, "position", Vector3(0,1,0), 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
		tween.chain().tween_property(selected_message, "scale", Vector3(0,0,0), 0.5).from(Vector3(2,2,2)).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
